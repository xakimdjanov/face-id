...
const TeacherSidebar = ({ isOpen, toggleSidebar }) => {
  ...
  const menuItems = [
    { name: "Boshqaruv paneli", path: "/teacher/dashboard", icon: <HiOutlineViewGrid size={22} /> },
    { name: "Guruhlarim", path: "/teacher/groups", icon: <HiOutlineUserGroup size={22} /> },
    { name: "Talabalar", path: "/teacher/students", icon: <HiOutlineAcademicCap size={22} /> },
  ];

  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0f172a] text-slate-300 border-r border-slate-800 shadow-2xl transform ${isOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col`}>
        ...
        <div>
          ...
          <div className="p-8 flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black text-xl shadow-lg shadow-blue-500/30">T</div>
              <span className="text-white font-bold text-lg tracking-tight">O'qituvchi</span>
            </div>
